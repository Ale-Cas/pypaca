...
RUN python -m venv /opt/pypaca-env
ENV PATH /opt/pypaca-env/bin:$PATH
ENV VIRTUAL_ENV /opt/pypaca-env

# Set the working directory.
WORKDIR /workspaces/pypaca/

# Install the run time Python dependencies in the virtual environment.
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/pypaca/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/pypaca/ && touch src/pypaca/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/pypaca/
