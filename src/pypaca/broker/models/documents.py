...
from pypaca.broker.enums import DocumentType, TradeDocumentSubType, TradeDocumentType
from pypaca.rest.models import ModelWithID

IPAddress = IPv4Address | IPv6Address


class AccountDocument(ModelWithID):
    """
...
class TradeDocument(ModelWithID):
    ...
    name: str
    type_: TradeDocumentType = Field(alias="type")
...
class W8BenDocument(BaseModel):
    """
    Represent the information normally contained in a W8BEN document.
    ...
    def root_validator(cls, values: dict) -> dict:
        """Validate the W8BenDocument model."""
        foreign_tax_set = "foreign_tax_id" in values and values["foreign_tax_id"] is not None
        tax_id_set = "tax_id_ssn" in values and values["tax_id_ssn"] is not None
        ftin_set = "ftin_not_required" in values and values["ftin_not_required"] is not None
